...
| [`deep_destructure.td`](deep_destructure.td) | `S05-pattern-matching` | Composing the existing pattern rules (G8 match, G19 variant, G24 tuple, §Records) at depth: variant of record of (tuple, sum). Spec composes cleanly. Surfaces that **record-field punning is intentionally not in v1**, so `User{ id, name }` patterns must be written as `User{ id: id, name: name }` — verbose but unambiguous. |
